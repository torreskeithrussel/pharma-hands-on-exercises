...
In this project we are working towards creating a demography summary table similar to this one: ![Demographic table](img/MiniProject4_demog_table.png). After completing the challenge, the updated table should be similar to this one:![Final demographic table](img/demog_summary_table.png)
...
```{r Challenge Items 1 & 2}
dm_allcomb2 <- bind_rows(dm_allcomb[4,],dm_allcomb[2,],dm_allcomb[1,],dm_allcomb[3,], dm_allcomb[5:8,])
dm_allcomb2
```

```{r Challenge Item 3}
varcat <- function(.dat=NULL,var=NULL) {
  ...
  var2 <- deparse(substitute(var))
  
  var_mrg_cnt <- var_n_cnt %>% 
    left_join(var_N_cnt, 
              by = c("TRT01AN", "TRT01A", var2))
  
  var_mrg_cnt2 <- var_mrg_cnt %>% 
    left_join(Big_N_cnt, 
              by = c("TRT01AN", "TRT01A"))
  
  var_mrg_cnt3 <- var_mrg_cnt2 %>% 
    left_join(small_n_cnt, 
              by = c("TRT01A", "TRT01AN", "SEX"))
  
  var_mrg_cnt3 <- ungroup(var_mrg_cnt3)
  
  var_data_new <- var_mrg_cnt3 %>% 
    mutate(perc_tot = round((var_total/N)*100, 1)) %>%                 
    mutate(perc_var = round((var_n/n)*100,1))
  
  var_pct <- var_data_new %>%
    mutate(perc_tchar = format(perc_tot, nsmall = 1)) %>%
    mutate(perc_achar = format(perc_var, nsmall = 1))
  
  var_n_pct <- var_pct %>%
    mutate(npct = paste(var_n, paste0("(", perc_achar, ")"))) %>% 
  ...
  var_cat <- rename(var_trans, category={{var}})
  var_cat %>%
    arrange(category)
...
adsl_ethnic <- varcat(adsl_eff, ETHNIC)
adsl_race <- varcat(adsl_eff,RACE)

adsl_categories <- bind_rows(dm_allcomb2, adsl_ethnic, adsl_race)
