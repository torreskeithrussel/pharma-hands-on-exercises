...
```{r}
plot2 <- plot1 + 
  labs(title = "Subject-Level Pulse Rate Trajectories Across Study Days",
       subtitle = "Safety Monitoring in the Xanomeline Clinical Trial",
       x = "Study Day",
...
```{r}
paramn_plot <- function(PARAMN=NULL) {
...
xLab <- quo_name(paramn_plot4$mapping$x)
yLab <- quo_name(paramn_plot4$mapping$y)
groupLab <- quo_name(paramn_plot4$mapping$group)

study <- uniqueVal(paramn_plot4$data$STUDYID)
measure <- uniqueVal(paramn_plot4$data$PARAM)
...
DiaBP_plot <- paramn_plot(2)
DiaBP_plot

Temp_plot <- paramn_plot(6)
Temp_plot
```
...
```{r}
advsmax <- import("https://github.com/phuse-org/phuse-scripts/raw/master/data/adam/cdisc/advsmax.xpt")
# advsmax
```
...
var <- advsmax %>%
    ...
    plot4 <- ggplot(data = var, mapping = aes(x=ADY, y=AVAL, group=USUBJID)) +
              geom_line() +
                    theme_bw() +
    ...
    xLab <- quo_name(varplot$mapping$x)
    yLab <- quo_name(varplot$mapping$y)
    groupLab <- quo_name(varplot$mapping$group)
    
    study <- uniqueVal(varplot$data$STUDYID)
    measure <- uniqueVal(varplot$data$PARAM)
