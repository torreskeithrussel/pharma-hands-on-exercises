...
5.  Merging "Big_N_cnt" and "small_n_cnt" before calculating percentages...
...
adae_saf <-   # adsl %>%
  filter( .data = adae, SAFFL == "Y" )
# adae_saf

adae_saf_bign <- adae_saf %>%
  group_by(TRTA, TRTAN) %>% 
  count(name = "N")
adae_saf_bign

adae_saf_bodsys <- adae_saf %>%
  group_by(TRTA, TRTAN, AEBODSYS) %>% 
  count(name = "n")
adae_saf_bodsys

adae_mrg_cnt <- adae_saf_bodsys %>%
  left_join(adae_saf_bign, by = c("TRTA", "TRTAN")) %>%
  mutate(perc = round(n/N*100, digits=1)) %>%
  mutate(perc_char = format(perc, nsmall=1)) %>%
  mutate(npct = paste(n, "(", perc_char, ")" ) ) %>%
  ungroup() %>%
...
adae_mrg_cnt %>%
  pivot_wider(names_from = TRTA, values_from = npct, values_fill = "0 ( 0.0 )")
```
