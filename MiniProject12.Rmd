...
test_that("calculate_small_n_cnt() output has expected dim, columns, and types", {
   ...
   expect_equal( dim(calculate_small_n_cnt(test_data)), 
                 c(length(unique(calculate_small_n_cnt(test_data)$TRT01A))
                   + length(unique(calculate_small_n_cnt(test_data)$SEX)), 4) )
})
```
...
test_that("sum of percentages per treatment equals 100", {
  aggregate_adsl_test_data <- aggregate(
    x = adsl_test_data$perc, 
    by=list(TRT01A=adsl_test_data$TRT01A), 
    FUN=sum)
  
  expect_identical(aggregate_adsl_test_data$x, c(100,100))
})
